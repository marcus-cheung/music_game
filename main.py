...
def setupMain():
    ...
    socketio.emit("add_playlist", [f"<button class='playlist' id='{playlist['id']}' value='{playlist['id']}''>{playlist['name']} onclick='add_playlist({playlist['id']},{playlist['name']})'</button>" for playlist in playlist_infos], room=request.sid)
